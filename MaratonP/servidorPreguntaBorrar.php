<!DOCTYPE html>
<?php 
/*Este archivo borra de la base de datos la pregunta recibida como argumento
...
$result=mysqli_query($conexion,"select * from superadmin t WHERE t.ID = '".  $id ."'") or die(mysqli_error()); 
	...
	$Clave=$_GET["Clave"]; //clave de la materia a la que pertenece la pregunta
$ID=$_GET["ID"]; //ID de la pregunta
...
<table class="CSS_Table_Example" align="center" width="69%" border="">
    <tr>
      <td colspan="4" scope="col">Pregunta Borrada! </td>
    </tr>
</table>
</div>
<?php
...
where Clase = '$Clave' AND ID = '$ID'")or die(mysqli_error());
	} 
